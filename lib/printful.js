...
export function assertVariantIdForProduct(product, size, options = {}) {
  ...
  if (!normalizedSize) {
    throw new Error("A valid poster size is required to resolve Printful variant IDs.");
  }
  ...
  const variants = product.printfulVariantIds || product.printfulVariantIDs || {};
  const variantLookup = {
  ...
  if (!variantId) {
    const name = product.name || product._id || "Unknown product";
    throw new Error(
      `Product "${name}" is missing a Printful variant ID for size ${size || normalizedSize}.`
    );
  }
