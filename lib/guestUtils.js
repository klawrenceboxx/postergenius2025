...
/**
 * Returns the stored guestId without creating one. Returns null if missing or expired.
 */
export function peekGuestId() {
  if (typeof window === 'undefined') return null;
  try {
    const raw = window.localStorage.getItem(GUEST_ID_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed?.id || isGuestIdExpired(parsed.createdAt)) return null;
    return parsed.id;
  } catch {
    return null;
  }
